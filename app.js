...
function lowest(values, endIndex, length) {
  const start = Math.max(0, endIndex - length + 1);
  let result = null;
  for (let i = start; i <= endIndex; i += 1) {
    const value = values[i];
...
function average(values, endIndex, length) {
  const start = Math.max(0, endIndex - length + 1);
  let sum = 0;
  let count = 0;
  for (let i = start; i <= endIndex; i += 1) {
...
function isBullishBreakout(candle, avgBody, avgVolume) {
  const body = Math.abs(candle.close - candle.open);
  return candle.close > candle.open
    && body >= (avgBody ?? 0) * 1.15
    && candle.volume >= (avgVolume ?? 0) * 1.2;
}
function hasThreeStrongReds(candles, index, avgBody20, avgVolume20) {
  if (index < 2) return false;
  for (let i = index - 2; i <= index; i += 1) {
    if (!isBullishBreakout(candles[i], avgBody20[i], avgVolume20[i])) return false;
  }
  return candles[index - 2].close < candles[index - 1].close && candles[index - 1].close <= candles[index].close;
}
function isLeftClean(candles, index, lookback = 60) {
  if (index < 3) return false;
  const currentWindowLow = Math.min(candles[index].low, candles[index - 1].low, candles[index - 2].low);
  const previousHigh = highest(candles.map((c) => c.high), index - 3, lookback);
  if (previousHigh == null) return true;
  return previousHigh < currentWindowLow * 0.97;
}
function hasCompressedMa(ma3, ma5, ma10, ma20, ma60, ma120, index, lookback = 8) {
  if (index < lookback) return false;
  for (let i = index - lookback + 1; i <= index; i += 1) {
    const values = [ma3[i], ma5[i], ma10[i], ma20[i], ma60[i], ma120[i]].filter((value) => value != null);
    if (values.length < 4) return false;
    const maxValue = Math.max(...values);
    const minValue = Math.min(...values);
    if ((maxValue - minValue) / Math.max(minValue, 0.0001) > 0.06) return false;
  }
  return true;
}
function findPriorHeavyBlackCandle(candles, index, lookback = 120) {
  let target = null;
  for (let i = Math.max(0, index - lookback); i < index; i += 1) {
    const candle = candles[i];
    if (candle.close >= candle.open) continue;
    if (!target || candle.volume > target.volume) {
      target = { high: candle.high, volume: candle.volume };
    }
  }
  return target;
}
...
function computeIndicator(candles) {
  ...
  const avgBody20 = sma(bodySizes, 20);
  ...
  let pendingReentryHigh = null;
  for (let i = 0; i < candles.length; i += 1) {
    const candle = candles[i];
    const breakout3 = hasThreeStrongReds(candles, i, avgBody20, avgVolume20);
    const above120 = ma120[i] != null && candle.close > ma120[i];
    const breaks120 = ma120[i] != null && candles[i - 1] && candles[i - 1].close <= ma120[i - 1] && candle.close > ma120[i];
    const leftClean = isLeftClean(candles, i);
    const recentHigh60 = i > 0 ? highest(highs, i - 1, 60) : null;
    const recentHigh30 = i > 0 ? highest(highs, i - 1, 30) : null;
    const recentLow120 = lowest(lows, i, 120);
    const recentDrawdown = recentHigh30 == null ? 0 : (recentHigh30 - candle.low) / Math.max(recentHigh30, 0.0001);
    const gapUp = candles[i - 1] ? candle.open > candles[i - 1].close * 1.02 : false;
    const lowVolumeRise = i >= 2
      && candles[i - 2].close < candles[i - 1].close
      && candles[i - 1].close <= candle.close
      && volumes[i] < (volMa5[i] ?? volumes[i]) * 0.95
      && volumes[i - 1] < (volMa5[i - 1] ?? volumes[i - 1]) * 0.95
      && (gapUp || candles[i - 1].open > candles[i - 2].close || candle.open > candles[i - 1].close);
    const compressedBefore = hasCompressedMa(ma3, ma5, ma10, ma20, ma60, ma120, i - 1);
    const wukongBreakout = compressedBefore
      && above120
      && ma3[i] != null && ma5[i] != null && ma10[i] != null && ma20[i] != null
      && ma3[i] >= ma5[i] && ma5[i] >= ma10[i] && ma10[i] >= ma20[i]
      && candle.volume >= (volMa20[i] ?? candle.volume) * 1.25;
    const blackSwanBuy = recentDrawdown >= 0.18
      && candle.close > candle.open
      && candle.close >= candle.high - (candle.high - candle.low) * 0.3
      && candle.volume >= (volMa20[i] ?? candle.volume) * 1.2;
    const firstIncense = gapUp
      && candle.close > candle.open
      && candle.volume >= (volMa20[i] ?? candle.volume) * 1.6;
    const priorHeavyBlack = findPriorHeavyBlackCandle(candles, i);
    const targetFromLow = recentLow120 == null ? null : recentLow120 * 3;
    const buyReasons = [];
    if (breakout3 && above120 && leftClean && (breaks120 || candle.volume >= (volMa20[i] ?? candle.volume) * 1.4)) buyReasons.push("沖天砲");
    if (breakout3 && above120 && !leftClean && recentHigh60 != null && candle.close > recentHigh60 * 1.01) buyReasons.push("第二春");
    if (lowVolumeRise) buyReasons.push("小蝦米");
    if (blackSwanBuy) buyReasons.push("黑天鵝");
    if (wukongBreakout) buyReasons.push("悟空翻身");
    if (firstIncense) buyReasons.push("搶頭香");
    if (pendingReentryHigh != null && candle.close > pendingReentryHigh * 1.01 && candle.volume >= (volMa20[i] ?? candle.volume) * 1.2) {
      buyReasons.unshift("第三把金鑰匙");
    }
    if (positionState !== "long" && buyReasons.length) {
      const primaryReason = buyReasons[0];
      const execBuy = candle.close;
      buySignals.push({ index: i, price: execBuy, reason: primaryReason });
      ...
      positionMeta = {
        ...
        targetPrice: primaryReason === "沖天砲" && targetFromLow != null ? targetFromLow : null,
        pressureHigh: primaryReason === "第二春" && priorHeavyBlack ? priorHeavyBlack.high : null,
        pressureVolume: primaryReason === "第二春" && priorHeavyBlack ? priorHeavyBlack.volume : null,
      };
      pendingReentryHigh = null;
      continue;
    }
    if (positionState === "long" && positionMeta) {
      positionMeta.peakPrice = Math.max(positionMeta.peakPrice, candle.high);
      const breaksMa10OnVolume = ma10[i] != null
        && candle.close < ma10[i]
        && candle.close < candle.open
        && candle.volume >= (volMa20[i] ?? candle.volume) * 1.5
        && positionMeta.peakPrice >= positionMeta.entryPrice * 1.08;
      const reachedChongTarget = positionMeta.reason === "沖天砲"
        && positionMeta.targetPrice != null
        && candle.high >= positionMeta.targetPrice;
      const secondSpringBlocked = positionMeta.reason === "第二春"
        && positionMeta.pressureHigh != null
        && candle.high >= positionMeta.pressureHigh
        && candle.close < candle.open
        && candle.volume <= (positionMeta.pressureVolume ?? Number.POSITIVE_INFINITY);
      const volumeShrinkStall = i >= 2
        && candle.volume <= (volMa5[i] ?? candle.volume) * 0.75
        && Math.abs(candle.close - candles[i - 1].close) / Math.max(candles[i - 1].close, 0.0001) < 0.015
        && ma10[i] != null
        && candle.close < ma10[i];
      let sellReason = null;
      if (reachedChongTarget) sellReason = "沖天砲第一目標價";
      else if (secondSpringBlocked) sellReason = "量大黑K壓力";
      else if (breaksMa10OnVolume) sellReason = "跌破10日線高檔放量";
      else if (volumeShrinkStall) sellReason = "量縮停滯";
      if (sellReason) {
        const execSell = candle.close;
        const pnl = lastBuyPrice == null ? null : ((execSell - lastBuyPrice) / lastBuyPrice) * 100;
        sellSignals.push({ index: i, price: execSell, pnl, reason: sellReason });
        pendingReentryHigh = positionMeta.peakPrice;
        lastBuyPrice = null;
        positionMeta = null;
        positionState = "flat";
      }
